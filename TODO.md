...
**Code Review**
- [x] Auth bypass audit — session validation, token scope enforcement, RBAC checks on every handler
- [x] PBAC enforcement audit — every action path goes through the policy engine
- [x] Missing error handling audit — external API calls, DB writes, job enqueue
- [x] Race condition audit — shared state, River workers, plugin registry
- [x] Event sourcing correctness — all state changes emit events, version conflicts handled
- [x] Cascade correctness — `CascadeRemoveJob` checks other active holds before removing
- [x] Credential leakage audit — nothing logged, returned in errors, or exposed in API responses
- [x] Plugin contract audit — idempotent hold operations, context cancellation respected
- [x] Security fix: TokenAuth now verifies token owner is_active before allowing access

**Coverage**
- [x] Add `github.com/testcontainers/testcontainers-go` to `go.mod`
- [x] `internal/rbac/` ≥ 90% — HasPermission granted/denied, wildcard expansion, assign/revoke idempotency
- [x] `internal/pbac/` ≥ 90% — all 17 policies have allow + deny test cases (existing tests cover all)
- [x] `internal/store/` ≥ 90% — AppendAndLoad ordering, VersionConflict, ClaimAndAck, Nack
- [x] `internal/auth/` ≥ 85% — session Create/Validate/expired/Delete, user GetOrCreate upsert, SetActive
- [x] `internal/legalhold/` ≥ 85% — Templates CRUD, CascadeStateMachine invalid transition
- [x] `plugins/*` ≥ 75% per plugin (existing plugin tests cover primary actions)
- [x] `internal/testutil/` — NewTestPool shared helper for DB-backed tests
- [x] `make coverage` target added to CI

**Go E2E Tests (`e2e/`)**
- [x] Public API token scoping: wrong scope = 403, correct scope = 200
- [x] Break-glass audit trail: incident row + event table verified
- [x] Approval workflow: action → pending approval → approve → approved status
- [x] Hold lifecycle: create → add custodian → verify → release
- [x] Auth inactive user: deactivate user → token calls return 401
- [x] Admin instances CRUD: create → list → delete → verify

**Playwright E2E (`frontend/e2e/`)**
- [x] auth.spec.ts — login page SSO button visible
- [x] theme.spec.ts — theme toggle switches data-theme attribute
- [x] dashboard.spec.ts — stub test (skipped without PLAYWRIGHT_BASE_URL)
- [x] playwright.config.ts — configured with webServer for local dev
- [x] Update `.github/workflows/ci.yml` — coverage + playwright jobs added
